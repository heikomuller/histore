...
class RIDocument(Document):
    ...
    data frame. Uses a provided counter to generate unique identifier for new
    rows.
    """
    def __init__(self, df, schema, row_counter):
        """Initialize the object properties. Creates identifier for all rows
        in the document based on the data frame row index. Uses the row counter
        to generate unique identifier for new rows.
        ...
        schema: list(histore.document.schema.Column)
            ...
            columns in the data frame.
        row_counter: int
            Counter that is used to generate unique identifier for new rows. It
            is assumed that the given counter value represents the first new
            row identifier. Consecutive row identifier are generated by
