...
class DefaultArchiveSerializer(ArchiveSerializer):
    ...
    def element_to_dict(self, node):
        ...
        if node.has_timestamp():
            obj[LABEL_TIMESTAMP] = str(node.get_timestamp())
        # Add key and positions if the element is keyed by values
        if not node.key is None:
            obj[LABEL_KEY] = node.key
            obj[LABEL_POSITIONS] = [
                self.value_to_dict(pos) for pos in node.positions
            ]
        # Add children
        children = list()
        for child in node.children:
            if child.is_value():
                children.append(self.value_to_dict(child))
            else:
                children.append(self.element_to_dict(child))
        obj[LABEL_NODES] = children
        return obj

    def value_to_dict(self, node):
...
class CompactArchiveSerializer(ArchiveSerializer):
    ...
    def element_to_dict(self, node, path=Path('')):
        """Get dictionary serialization for an archive element node.
        ...
        # Add timestamp only if the element has a local timestamp
        if node.has_timestamp():
            meta[LABEL_TIMESTAMP] = str(node.get_timestamp())
        # Add key and positions if the element is keyed by values
        if not node.key is None:
        ...
        for child in node.children:
            if child.is_value():
                ...
                obj[self.mapping[LABEL_VALUE]].append(self.value_to_dict(child))
            else:
                ...
                if not target_path.to_key() in self.key_paths:
                    ...
                    obj[child.label].append(self.element_to_dict(child, path=target_path))
        return obj
    ...
    def value_to_dict(self, node):
        ...
        if node.has_timestamp():
            return {
                ...
                LABEL_TIMESTAMP: str(node.get_timestamp())
            }
        else:
            return node.value
